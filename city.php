...
<body>
    ...
    <div class="container">
        <div class="card text-center">
            <?php
                $sql2 = "SELECT geocities.GeoNameID, geocities.AsciiName, geocities.Latitude, geocities.Longitude, geocities.Population, geocities.Elevation, geocountries.CountryName, geocountries.ISONumeric AS CountryISO FROM geocities INNER JOIN geocountries ON geocities.CountryCodeISO = geocountries.ISO WHERE geocities.GeoNameID = $city_id";
                $result2 = $conn->query($sql2);
    
                if ($result2->num_rows > 0) {
                    // output data of each row
                    while($row = $result2->fetch_assoc()) {
                        echo "<h1 style='padding-top: 20px;'>".$row["AsciiName"]."</h1>
                            <p>Population: ".number_format($row["Population"])."</p>
                            <p>Elevation: ".number_format($row["Elevation"])."&nbsp;m</p>
                            <p><a href='country.php?id=".$row["CountryISO"]."'>".$row["CountryName"]."</a></p><br>
                            <div class='well'>
                                <iframe 
                                    width='600' 
                                    height='450' 
